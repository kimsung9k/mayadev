...
def loadRelativeCtls( ikCtl ):
    ...
    if ikCtl.find( 'Ctl_Ik' ) != -1:
        ns = ikCtl.replace( 'Ctl_Ik', '' )
        
        lns = 'LA'
        rns = 'RA'
        if ns.find( 'LA:' ) != -1:
            lns = ns
            rns = ns.replace( 'LA:', 'RA:' )
            CtlInfo.leftCtls = cmds.ls( lns+'Ctl_*', type='transform' )
            CtlInfo.rightCtls = cmds.ls( rns+'Ctl_*', type='transform' )
        elif ns.find( 'RA:' ) != -1:
            rns = ns
            lns = ns.replace( 'RA:', 'LA:' )
            CtlInfo.leftCtls = cmds.ls( lns+'Ctl_*', type='transform' )
            CtlInfo.rightCtls = cmds.ls( rns+'Ctl_*', type='transform' )
...
def setMirrorObjectOnce( target ):
    ...
    if target in CtlInfo.leftCtls:
        otherTarget = target.replace( 'LA:', 'RA:' )
    elif target in CtlInfo.rightCtls:
        otherTarget = target.replace( 'RA:', 'LA:' )
    if not otherTarget: return None
        
    targetMtx = cmds.getAttr( target+'.m' )
    cmds.xform( otherTarget, os=1, matrix=targetMtx )
...
def setMirrorObject( fromList, toList ):
    ...
    for i in range( len( fromList ) ):
        ...
        if fromCtl.find( 'aimConstraint' ) != -1: continue
        if fromCtl.find( 'putObj' ) != -1: continue
