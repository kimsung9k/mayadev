...
def createSlidingDeformer( sels ):
    ...
    if len( sels ) > 1:
        ...
        for other in others:
            ...
            if srcCons:
                cmds.connectAttr( srcCons[0], deformer+'.baseMesh' )
                smoothMeshNode = cmds.createNode( 'polySmoothFace' )
                cmds.connectAttr( trGeoNode+'.outputGeometry', smoothMeshNode +'.inputPolyMesh' )
                cmds.connectAttr( smoothMeshNode+'.output', deformer+'.baseMesh' )
            else:
                selShapes = cmds.listRelatives( other, s=1 )
                for shape in selShapes:
                    if cmds.getAttr( shape+'.io' ):
...
uiCmd_createSlidingDeformer = """import deformer.cmdModel
import maya.cmds as cmds
sels = cmds.ls( sl=1 )
deformer.cmdModel.createSlidingDeformer( sels )"""