...
class Window:
    ...
    def loadLeftWing(self, *args ):
        ...
        if selTarget.find( 'Ctl_Ik' ) != -1:
            cmds.textField( self.fieldLeft, e=1, tx= selTarget )
            cmds.textField( self.fieldRight, e=1, tx= selTarget.replace( 'LA:', 'RA:' ) )
            cmdModel.loadRelativeCtls( selTarget )
        else:
            cmds.warning( 'Select "LA:Ctl_Ik"' )
    ...
    def loadRightWing(self, *args ):
        ...
        selTarget = cmds.ls( sl=1 )[-1]
        if selTarget.find( 'Ctl_Ik' ) != -1:
            cmds.textField( self.fieldLeft, e=1, tx= selTarget.replace( 'RA:', 'LA:' )  )
            cmds.textField( self.fieldRight, e=1, tx= selTarget )
            cmdModel.loadRelativeCtls( selTarget )
        else:
            cmds.warning( 'Select "RA:Ctl_Ik"' )
