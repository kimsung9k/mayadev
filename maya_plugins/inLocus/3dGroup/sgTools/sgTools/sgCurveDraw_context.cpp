...
MStatus sgCurveDraw_ToolCommand::redoIt()
{
	...
	if( m_startParam > m_endParam )
	{
		...
		for( unsigned int i=0; i< m_curve_points.length(); i++ )
		{
	...
	if( !m_editMode )
	{
		...
		int renumSpans = (int)(spans * fnCurveCreate.length() / curveLength);
...
MStatus sgCurveDraw_context::doPtrMoved(MEvent& evt)
{
	return MS::kSuccess;
}
MStatus sgCurveDraw_context::doPtrMoved(MEvent &evt, MHWRender::MUIDrawManager &drawManager, MHWRender::MFrameContext const &frameContext)
{
	return MS::kSuccess;
}