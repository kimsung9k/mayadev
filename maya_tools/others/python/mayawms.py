...
class mayaviewport_NewPreviewWindow():
    def __init__(self, name, width, height, camera):
        self.name = name
        self.width = width
        self.height = height
        if camera is None:
            self.camera = cmds.modelEditor(mayaviewport_activeView(), q=True, camera=True)
        else:
            self.camera = camera
        self.window = mayaui_New(cmds.window, self.name, sizeable=False, width=self.width, height=self.height)
        form = cmds.formLayout()
        self.Editor = mayaui_New(cmds.modelEditor, name+'Editor', camera = self.camera, activeView=True)
...
class UI:
    ...
    def previewSceneEv(self, _):
        ...
        makePreview(
            seqpath, mp4path,
            startf, endf,
            mayaoption_load('WMS_Preview_Width', 640), mayaoption_load('WMS_Preview_Height', 360),
            None,
            soundfile
        )
        # mp4파일을 연다.
        ...
        with self.Window:
            with mayaui_New(cmds.columnLayout, 'MainLayout'):
                with mayaui_New(cmds.rowLayout, 'SizeRow', numberOfColumns=3):
                    ...
                    w, h = mayaoption_load('WMS_Preview_Width', 640), mayaoption_load('WMS_Preview_Height', 360)
...
class ProgressWindow():
    def __init__(self, title, maxv):
        ...
        self.Window.edit(width=640, height=1)
        cmds.showWindow()
        with self.Window:
            with mayaui_New(cmds.columnLayout, adj=True):
    ...
    if os.path.exists(w.mb):
        ...
        if w.process not in prior:
            ...
            if answer == u'새로 시작':
                ...
                downloadWorks.extend(downloadRelatedWorks(workFromPath(priormb, w.hostip)))
...
def makePreview(seqpath, mp4path, startf, endf, width, height, camera, soundfile):
    ...
    with mayaviewport_NewPreviewWindow('WMS_PreviewWindow', width, height, camera):
        cmds.select(cl=1)
...
def screenshot(w):
    ...
    with mayarender_OutAsJpeg():
        ...
        with PrevWin:
            ...
            mayaplayblast_oneshot(image, mayascene_animationStart(), 640, 360)
