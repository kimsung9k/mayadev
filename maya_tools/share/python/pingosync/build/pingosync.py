...
class Window_Login( QDialog ):
    ...
    def createAcount(self):
        ...
        createAcount = Window_CreateAcount( Models.ControlBase.mayawin )
        createAcount.show()
    ...
    def getIdAndPass(self):
        ...
        if False:
            ...
            Models.ControlBase.mainui = Window( Models.ControlBase.mayawin )
            Models.ControlBase.mainui.show()
...
class Window( QMainWindow ):
    ...
    def __init__(self, *args, **kwargs ):
        ...
        treeWidget.itemExpanded.connect( commands.TreeWidgetCmds.updateTaskHierarchy )
        treeWidget.itemCollapsed.connect( commands.TreeWidgetCmds.updateTaskHierarchy )
        button_serverPath.clicked.connect( commands.ProjectControl.resetServerPath )
        button_localPath.clicked.connect( commands.ProjectControl.resetLocalPath )
        
        treeWidget.setFont( QFont( "", 9, QFont.Light ) )
        
        if commands.ProjectControl.getCurrentProjectName():
        ...
        Models.ControlBase.uiTreeWidget = treeWidget
    ...
    def loadContextMenu(self, *args ):
        ...
        serverPath = commands.FileControl.getCurrentServerProjectPath()
        localPath  = commands.FileControl.getCurrentLocalProjectPath()
        targetPath_inServer = serverPath + elsePath
        targetPath_inLocal  = localPath  + elsePath
        
        menu = QMenu( Models.ControlBase.uiTreeWidget )
        
        enableOpenFile = commands.QueryCmds.isEnableOpen( targetPath_inServer, targetPath_inLocal )
        ...
        if enableReference:
            menu.addAction( "레퍼런스".decode( "utf-8" ), commands.ContextMenuCmds.reference )
        menu.addAction("폴더열기".decode( 'utf-8'), commands.ContextMenuCmds.openFileBrowser_local )
        ...
        separator = QAction( self ); separator.setSeparator( True );menu.addAction( separator )
        menu.addAction("새로고침".decode( 'utf-8'), commands.ContextMenuCmds.refresh )
    ...
    def loadProject(self):
        ...
        commands.ProjectControl.setCurrentProjectName( currentText )
        self.lineEdit_serverPath.setText( commands.FileControl.getCurrentServerProjectPath() )
        self.lineEdit_localPath.setText( commands.FileControl.getCurrentLocalProjectPath() )
    ...
    def show_manageProject(self):
        ...
        Models.ControlBase.manageui = self.window_manageProject
    ...
    def loadUIInfo( self ):
        ...
        Models.ControlBase.mainui.move( posX, posY )
        Models.ControlBase.mainui.resize( width, height )
...
def show( evt=0 ):
    ...
    Models.ControlBase.mainui = Window( Models.ControlBase.mayawin )
    Models.ControlBase.mainui.loadUIInfo()
    Models.ControlBase.mainui.show()
