...
class WinA_Global:
    ...
    winName = sgBModel_sgPWindow.sgPWindow_file_camera_export_winName
    title   = "Export Camera"
    width   = 500
    height  = 50
    titleBarMenu = True
    ...
    infoFilePath   = sgBFunction_fileAndPath.getPathInfo_sgPWindow_file_camera()
...
class WinA_Cmd:
    ...
    def read_windowInfo():
        ...
        upFolderNum, addPath = sgBModel_aniScene.exportCameraPathFromAni
        sceneName = cmds.file( q=1, sceneName=1 )
        sceneFolder = '/'.join( sceneName.split( '/' )[:-1+upFolderNum] )
        exportPath = sceneFolder + addPath
        
        sceneNum = sceneName.split( '/' )[-1].split( '_' )[0]
        camFileName = sgBModel_aniScene.exportCameraFileName.replace( "%SCENENUM%", sceneNum )
        
        exportPath += "/" + camFileName
...
mc_showWindow = """import sgPWindow_file_camera_export
sgPWindow_file_camera_export.WinA().create()"""