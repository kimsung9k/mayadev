...
def getMMatrixFromMtxList( mtxList ):
    mMatrix = om.MMatrix()
    om.MScriptUtil.createMatrixFromList( mtxList, mMatrix )
    return mMatrix



def checkComponentIsMoved( shape ):
    
    if cmds.nodeType( shape ) == 'nurbsSurface':
        fnShape = om.MFnNurbsSurface( getMObject( shape ) )
        numCVsU = fnShape.numCVsInU()
        numCVsV = fnShape.numCVsInV()
        
        for u in range( numCVsU ):
            for v in range( numCVsV ):
                compV = om.MVector( *cmds.getAttr( shape+'.cv[%d][%d]' %( u, v ) )[0] )
                if compV.length() > 0:
                    return True
                
    elif cmds.nodeType( shape ) == 'nurbsCurve':
        fnShape = om.MFnNurbsCurve( getMObject( shape ) )
        numCVs = fnShape.numCVs()
        
        for i in range( numCVs ):
            compV = om.MVector( *cmds.getAttr( shape+'.cv[%d]' % i )[0] )
            if compV.length()  > 0:
                return True
    elif cmds.nodeType( shape ) == 'mesh':
        fnShape = om.MFnMesh( getMObject( shape ) )
        numVtx = fnShape.numVertices()
        
        for i in range( numVtx ):
            compV = om.MVector( *cmds.getAttr( shape+'.vtx[%d]' % i )[0] )
            if compV.length() > 0:
                return True
    return False
...
def getShapeFromObject( node, typ='' ):
    
    if not typ:
        typList = ['mesh', 'nurbsCurve', 'nurbsSurface']
    else:
        typList = [typ]
    
    if cmds.nodeType( node ) in typList:
        return node
    ...
    if cmds.nodeType( node ) == 'transform':
        ...
        for shape in shapes:
            if cmds.nodeType( shape ) in typList:
