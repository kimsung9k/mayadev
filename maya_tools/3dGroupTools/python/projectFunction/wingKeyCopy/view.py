...
class Window:
    ...
    def loadLeftWing(self, *args ):
        ...
        if selTarget.find( 'L_wrist_CTL' ) != -1:
            cmds.textField( self.fieldLeft, e=1, tx= selTarget )
            cmds.textField( self.fieldRight, e=1, tx= selTarget.replace( 'L_', 'R_' ) )
            cmdModel.loadRelativeCtls( selTarget )
        else:
            cmds.warning( 'Select "L_wrist_CTL"' )
    ...
    def loadRightWing(self, *args ):
        ...
        selTarget = cmds.ls( sl=1 )[-1]
        if selTarget.find( 'R_wrist_CTL' ) != -1:
            cmds.textField( self.fieldLeft, e=1, tx= selTarget.replace( 'R_', 'L_' )  )
            cmds.textField( self.fieldRight, e=1, tx= selTarget )
            cmdModel.loadRelativeCtls( selTarget )
        else:
            cmds.warning( 'Select "R_wrist_CTL"' )
