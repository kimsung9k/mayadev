...
void SGDagNodeMesh::setDatasComopnentRelationship()
{
	...
	for (int i = 0; i < m_numVertices; i++)
	{
		printf("vertexToPolygons[%d] : ", i );
		for (int j = 0; j < m_vertexToPolygonsMap[i].length(); j++)
		{
			printf("%d ", m_vertexToPolygonsMap[i][j]);
		}
		printf("\n");
	}
	...
	for (int i = 0; i < m_numPolygons; i++)
	{
		printf("polygonToVertices[%d] : ", i);
		for (int j = 0; j < m_polygonToVerticesMap[i].length(); j++)
		{
			printf("%d ", m_polygonToVerticesMap[i][j]);
		}
		printf("\n");
	}*/
}
